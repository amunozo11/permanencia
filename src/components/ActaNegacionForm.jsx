...
const handleChange = (e) => {
  ...
  // Solo letras y espacios para estos campos
  const soloLetras = ["nombre_estudiante", "programa", "docente_permanencia", "documento_expedido_en"];
  if (soloLetras.includes(name)) {
    filteredValue = value.replace(/[^A-Za-zÁÉÍÓÚáéíóúñÑ ]/g, "");
  }

  // Solo números con longitud máxima para ciertos campos
  const soloNumerosMax10 = ["documento_numero", "documento_firma_estudiante"];
  const fechas = {
    fecha_firma_dia: 2,
    fecha_firma_mes: 2,
    fecha_firma_anio: 4,
  ...
  if (soloNumerosMax10.includes(name)) {
    filteredValue = value.replace(/\D/g, "").slice(0, 10);
  } else if (Object.keys(fechas).includes(name)) {
    filteredValue = value.replace(/\D/g, "").slice(0, fechas[name]);
  }
